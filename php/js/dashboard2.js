...
function initRows() {
    ...
    dataRows.forEach((r, idx) => {
        ...
        // lecture JSON data-impayes / data-remises
        let impayes = [];
        let remises = [];
...
function updateCountersAndTotals(visibleIndices) {
    ...
    // Mise à jour du camembert (par motifs d'impayés)
    updatePieChart();
}
...
function renderPage() {
    ...
    rowsData.forEach((item, i) => {
        if (slice.includes(i)) item.rowEl.style.display = 'table-row';
    });
...
function applyFilterAndRefresh() {
    ...
    rowsData.forEach((item, idx) => {
        ...
        // Recherche aussi dans les libellés de remises (numéro de remise par ex.)
        const remiseMatch = item.remises.some(r =>
            (r.libelle || '').toLowerCase().includes(q)
        );

        if (q && !hay.includes(q) && !remiseMatch) return;
...
// ---------- Exports CSV / XLS / PDF (tableau principal ou sidebar) ----------
function exportCSV(forSidebar = false, sidebarType = 'impayes') {
    ...
    a.href = url;
    a.download = forSidebar ? `${sidebarType.toUpperCase()}_EXPORT.csv` : 'REMISSES_EXPORT.csv';
...
async function exportPDF(forSidebar = false, sidebarType = 'impayes') {
    ...
    // Ajout des graphiques : main chart + camembert
    let startY = 30;
    try {
    ...
    if (forSidebar) {
        ...
        doc.autoTable({
            head: [['Date', 'Date limite', 'Libellé', 'Montant']],
            body,
            startY,
            styles: { fontSize: 9 }
        });
        ...
        doc.autoTable({
            head: [['DATE', 'INTITULÉ', 'N° SIRET', 'MONTANT']],
            body,
            startY,
            styles: { fontSize: 9 }
        });
...
// ---------- Chart.js : graphique d'évolution + camembert motifs ----------
...
function computeChartDatas() {
    ...
    rowsData.forEach(r => {
        ...
        if (r.montant >= 0) {
            caMap[r.dateText] += r.montant;
        } else {
            impMap[r.dateText] += Math.abs(r.montant);
        }
    });
...
// Camembert : répartition des motifs d'impayés
...
// Changement de type (courbe / histogramme) pour le graphique principal
...
// Mise à jour des datasets du graphique principal (CA / Impayés)
...
// ---------- Camembert : répartition par motifs d'impayés ----------
function updatePieChart() {
    const motifMap = {};

    // On utilise les lignes filtrées ; si aucune, on prend toutes les lignes
    const indices = filteredIndexList.length
        ? filteredIndexList
        : rowsData.map((_, idx) => idx);

    indices.forEach(i => {
        const item = rowsData[i];

        // Chaque impayé de cette ligne
        item.impayes.forEach(imp => {
            const motif = (imp.motif || imp.libelle || 'Sans motif').trim();
            const montant = parseFloat(imp.montant) || 0;

            if (!motifMap[motif]) motifMap[motif] = 0;
            motifMap[motif] += Math.abs(montant);
        });
    });

    const entries = Object.entries(motifMap);

    if (entries.length === 0) {
        pieChartInstance.data.labels = ['Aucun impayé'];
...
// Boutons d'export dans la sidebar (ici pour les impayés, tu peux dupliquer pour remises si besoin)
document.getElementById('export-sidebar-csv').addEventListener('click', () => {
    exportCSV(true, 'impayes');
});
document.getElementById('export-sidebar-xls').addEventListener('click', () => {
    exportXLS(true, 'impayes');
});
document.getElementById('export-sidebar-pdf').addEventListener('click', () => {
    exportPDF(true, 'impayes');
});
...
// ---------- Binding des en-têtes pour le tri ----------
...
// ---------- Récupération de toutes les remises (pour le modal global) ----------
...
// Modal global remises (PO)
document.getElementById('close-global-remises').addEventListener('click', () => {
...
function boot() {
    ...
    chartInstance.data.datasets[0].data = initial.caArr;
    chartInstance.data.datasets[1].data = initial.impArr;
    chartInstance.update();
